...
export class AnomalyCalculator {
    ...
    constructor(climatologyData) {
        ...
        if (climatologyData.length !== 366) {
            console.warn(
                `Climatology data should have 366 days (including Feb 29), ` + `but got ${climatologyData.length} days`
            );
        }
...
function runTests() {
    ...
    processed.forEach((day) => {
        console.log(
            `  ${day.date}: ${day.temperature}°C, Z=${day.zScore.toFixed(2)}σ, Anomaly=${day.anomaly > 0 ? '+' : ''}${day.anomaly.toFixed(1)}°C, Class=${day.classification}`
        );
    });
